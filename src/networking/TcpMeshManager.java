...
public class TcpMeshManager {
    ...
    private void handleNewConnection(Socket socket) {
        try {
            ...
            out.writeObject(new GameMessage(GameMessage.Type.HEARTBEAT, myPort, context.myIp));
            out.flush();
    ...
    public void sendNack(String targetPeerId, long sequenceNumber) {
        sendToPeer(targetPeerId, new GameMessage(GameMessage.Type.NACK, myPort, context.myIp, String.valueOf(sequenceNumber)));
    }
        ...
        while (running) {
            try {
                ...
                GameMessage hb = new GameMessage(
                    GameMessage.Type.HEARTBEAT, myPort, context.myIp, "Pulse"
                );
                broadcastToAll(hb);
